...
z <- outer(x, y, f)
z[is.na(z)] <- 1
trans = persp(x, y, z, theta=120, phi = 20, expand = 0.5, col = 'White', border = 'orange')
@
...
grid.newpage()
vp = plotViewport( xscale = range(xy$x, na.rm = TRUE), yscale = range(xy$y, na.rm = TRUE))
pushViewport(vp)
grid.polygon(xy$x, xy$y, id.lengths = rep(4, length(xy$x)/4), default.units = 'native', gp = gpar(fill = 'orange'))
...
vp = plotViewport( xscale = range(xy$x, na.rm = TRUE), yscale = range(xy$y, na.rm = TRUE))
pushViewport(vp)
grid.polygon(xyCoor$x, xyCoor$y, id.lengths = rep(4, length(xy$x)/4),
                default.units = 'native',
                gp = gpar( fill = 'orange'))


trans = persp(x, y, z, theta = 30, phi = 30, expand = 0.5, col = "orange", axes = FALSE, box = FALSE)
