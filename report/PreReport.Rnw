...
As we see, the \texttt{grid.ls()} returns a list of grid grobs of the pervious plot that been redrawn by grid. there is one element called "graphics-plot-1-bottom-axis-labels-1" which is the label of the bottom axis. In \texttt{grid}, there are serval function that used for mainpulate this grob. For example, if the user wants to rotate the labels of the bottom axis by 30 degrees and changes the color from default to orange, then the following code mainpulate this changes.
...
\begin{enumerate}
  \item 1. read and understand the C code, do the direct translation from C code to R code. \\
  \item ...
\end{enumerate}
